...
describe('storyCache', () => {
  ...
  const mockStory = { id: 123, title: 'Test Story', points: 100 };
  const mockComments = [
  ...
  describe('getCachedStory', () => {
    ...
    it('returns null for null/undefined storyId', () => {
      expect(getCachedStory(null)).toBeNull();
      expect(getCachedStory(undefined)).toBeNull();
    });
  ...
  describe('setCachedStory', () => {
    ...
    it('stores different stories separately', () => {
      const story1 = { id: 1, title: 'Story 1' };
      const story2 = { id: 2, title: 'Story 2' };
      
      setCachedStory(1, story1, []);
      setCachedStory(2, story2, []);
    ...
    it('overwrites existing cache for same storyId', () => {
      setCachedStory(123, { title: 'Old' }, []);
      setCachedStory(123, { title: 'New' }, []);
...
describe('sessionStorage - List Session State', () => {
  ...
  describe('saveListSessionState', () => {
    ...
    it('converts Set to Array for storage', () => {
      ...
      const stored = JSON.parse(sessionStorage.getItem('hackertok_session_top'));
  ...
  describe('clearListSessionState', () => {
    ...
    it('does not throw for non-existent session', () => {
      expect(() => clearListSessionState('nonexistent')).not.toThrow();
    });
    ...
    it('handles special characters in storyType', () => {
      saveListSessionState('top/special', mockState);
      
      const result = getListSessionState('top/special');
      expect(result).not.toBeNull();
    });
  });
});
