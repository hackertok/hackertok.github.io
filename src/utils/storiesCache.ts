 ...
 */
export function getCachedStories(type: StoryType): { stories: Story[]; timestamp: number; isStale: boolean } | null {
  try {
    ...
    const { stories, timestamp } = JSON.parse(cached) as { stories: Story[]; timestamp: number };
 ...
 */
export function setCachedStories(type: StoryType, stories: Story[]): void {
  try {
    const key = `${CACHE_KEY_PREFIX}${type}`;
    const data = {
      stories,
 ...
 */
export function clearStoriesCache(type?: StoryType): void {
  try {
    if (type) {
 ...
 * @returns {boolean}
 */
export function isCacheFresh(type: StoryType): boolean {
