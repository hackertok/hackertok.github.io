 ...
 */
interface UseStoryWithCommentsOptions {
...
interface UseStoryWithCommentsResult {
  story: Story | null;
...
export function useStoryWithComments(storyId: number | string, { initialStory = null, skipOrderingCompletion = false, isPriority = true, deferComments = false }: UseStoryWithCommentsOptions = {}): UseStoryWithCommentsResult {
  ...
  const shouldSkipAbort = useEffectEvent((effectStoryId: number | string) => {
  ...
  useEffect(() => {
    ...
    async function loadComments(storyData: Story | null, isBackgroundReorder = false) {
