/* eslint-disable vitest/expect-expect -- Many tests here verify side effects (queueing, caching, cancellation) without explicit assertions */
...
describe('usePrefetchStory', () => {
  ...
  describe('startPrefetch and stopPrefetch', () => {
    ...
    it('skips stories with fresh cache', () => {
      ...
      setCachedStory(12345, createStory({ id: 12345, title: 'Cached' }), [] as Comment[]);
...
describe('usePrefetchStories', () => {
  ...
  const mockStories = [
    ...
    createStory({ id: 3, title: 'Story 3' }),
    createStory({ id: 4, title: 'Story 4' }),
    createStory({ id: 5, title: 'Story 5' }),
  ];
  ...
  it('skips stories with fresh cache', () => {
    // Pre-populate cache
setCachedStory(2, createStory({ id: 2, title: 'Story 2' }), [] as Comment[]);
  ...
  it('handles undefined stories', () => {
    expect(() => {
      renderHook(() => usePrefetchStories(0, undefined as unknown as Story[], 3));
    }).not.toThrow();
  });
});
