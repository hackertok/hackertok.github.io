...
The private VAPID JWK remains a Cloudflare Secret; it is never copied to GitHub.
...
`worker/.dev.vars` and fill it with a disposable matching VAPID pair. The real
production private JWK must not be used locally.
...
1. Let staging finish bootstrap and confirm `/health/ready` returns `204`, the
   ...
   without deliveries.
2. Enroll through HackerTok's transient action and invoke the fixed self-test
   endpoint on Chrome/Android, Firefox, Edge, macOS Safari, and an installed
...
- `GET /health/ready`: returns `204` only after bootstrap reaches `ACTIVE` and
  the public/private VAPID configuration matches; otherwise `503`. The body is
  always empty, and `x-hackertok-release` identifies the deployed build.
- `GET /v1/push/config`: returns the enabled state, strict threshold, VAPID key
  ID, and public application-server key.
...
boundary. Apple relay hosts are admitted through the documented
...
- `408/429/5xx` or timeout: bounded exponential retry, honoring capped
  `Retry-After`.
- `401/403`: pause sending behind a VAPID/provider circuit breaker; do not
  delete subscriptions.
- `400/413` or redirects: pause as a sender/payload fault.
- Other `4xx`: terminalize only that delivery.
...
messages, stale leases, and page progress without a next wake-up.
...
Initial admission is atomically capped at 350 active installations. The release
gate is zero CPU-limit errors with comfortable margin below the Free plan's
10 ms CPU limit for detector, fan-out, and single-delivery handlers.
...
transactionally in the D1 ledger, so config checks do not scan subscriptions.
...
tombstones, with title/score details scrubbed after 90 days.
